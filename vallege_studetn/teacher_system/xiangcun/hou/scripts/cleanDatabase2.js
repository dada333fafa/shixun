...
mongoose.connect(process.env.MONGODB_URI).then(async () => {
  ...
  try {
    // 删除 teachers 集合的旧 userId_1 索引
    await mongoose.connection.db.collection('teachers').dropIndex('userId_1');
    console.log('Dropped userId_1 index from teachers');
  } catch (e) {
    console.log('Teachers userId_1 index not found, skipping:', e.message);
  }
  
  try {
    // 删除 students 集合的旧 userId_1 索引
    await mongoose.connection.db.collection('students').dropIndex('userId_1');
    console.log('Dropped userId_1 index from students');
  } catch (e) {
    console.log('Students userId_1 index not found, skipping:', e.message);
  }
