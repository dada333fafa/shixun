...
api.interceptors.response.use(
  ...
  (error) => {
    if (error.response) {
      ...
      // 统一错误格式，兼容 { msg: '错误信息' } 和 { message: '错误信息' }
...
export const matchAPI = {
  // 创建匹配（学生发送请求）
  createMatch(teacherId, message) {
    return api.post('/matches/request', { teacherId, message: message || '' })
  },
...
export const psychologicalAPI = {
  // 创建心理记录
  createRecord(data) {
    return api.post('/psychological/assess', data)
  },
  
  // 获取心理状态列表
  getStatus(params) {
    return api.get('/psychological/history', { params })
  }
}
